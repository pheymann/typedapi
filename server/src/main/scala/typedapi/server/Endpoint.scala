...
abstract class Endpoint[El <: HList, KIn <: HList, ROut, VIn <: HList, F[_], Out](val extractor: RouteExtractor.Aux[El, KIn, VIn, HNil, ROut]) {
...
final class EndpointDefinition[El <: HList, KIn <: HList, ROut, VIn <: HList, Fun[_[_]], Out](extractor: RouteExtractor.Aux[El, KIn, VIn, HNil, ROut], 
  ...
  /** Restricts type of parameter `f` to a function defined by the given API:
    ...
    */
  def to[F[_]](f: Fun[F]): Endpoint[El, KIn, ROut, VIn, F, Out] = 
    new Endpoint[El, KIn, ROut, VIn, F, Out](extractor) {

...
sealed trait MethodCall extends ApiOp
sealed trait GetCall extends MethodCall
sealed trait PutCall extends MethodCall
sealed trait PutWithBodyCall extends MethodCall
sealed trait PostCall extends MethodCall
sealed trait PostWithBodyCall extends MethodCall
sealed trait DeleteCall extends MethodCall
...
trait ApiTransformer {
  ...
  implicit def pathElementTransformer[S, El <: HList, KIn <: HList, VIn <: HList, M <: MethodCall, Out] = 
    at[PathElement[S], (El, KIn, VIn, M, Out), (S :: El, KIn, VIn, M, Out)]

  implicit def segmentElementTransformer[S <: Symbol, A, El <: HList, KIn <: HList, VIn <: HList, M <: MethodCall, Out] = 
    at[SegmentParam[S, A], (El, KIn, VIn, M, Out), (SegmentInput :: El, S :: KIn, A :: VIn, M, Out)]

  implicit def queryElementTransformer[S <: Symbol, A, El <: HList, KIn <: HList, VIn <: HList, M <: MethodCall, Out] = 
    at[QueryParam[S, A], (El, KIn, VIn, M, Out), (QueryInput :: El, S :: KIn, A :: VIn, M, Out)]

  implicit def queryListElementTransformer[S <: Symbol, A, El <: HList, KIn <: HList, VIn <: HList, M <: MethodCall, Out] = 
    at[QueryParam[S, List[A]], (El, KIn, VIn, M, Out), (QueryInput :: El, S :: KIn, List[A] :: VIn, M, Out)]

  implicit def headerElementTransformer[S <: Symbol, A, El <: HList, KIn <: HList, VIn <: HList, M <: MethodCall, Out] = 
    at[HeaderParam[S, A], (El, KIn, VIn, M, Out), (HeaderInput :: El, S :: KIn, A :: VIn, M, Out)]

  implicit def rawHeadersElementTransformer[El <: HList, KIn <: HList, VIn <: HList, M <: MethodCall, Out] = 
